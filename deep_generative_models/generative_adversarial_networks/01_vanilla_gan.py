...
parser.add_argument('--output-dir', default='logs/vanila_gan')
...
for epoch in range(200):
    for idx, data in tqdm(enumerate(dataloader), total=len(dataloader), desc='epoch #{:>3d}'.format(epoch), leave=False):
        ...
        if idx % 100 == 0:
            torchvision.utils.save_image(fake_data.detach().reshape(batch_size, 1, 28, 28), './{}/fake.png'.format(opt.output_dir, idx), normalize=True)