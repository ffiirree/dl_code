...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.embeds = nn.Embedding(num_classes, opt.nz)
...
class Discriminatar(nn.Module):
    def __init__(self):
        ...
        self.aux_layer = nn.Sequential(
            nn.Linear(opt.ndf * 8 * feature_map_size ** 2, num_classes),
            nn.Softmax(dim=1)
            # state size : (batch_size, num_classes)
        )
...
for epoch in range(200):
    for idx, (images, labels) in tqdm(enumerate(dataloader), total=len(dataloader), desc='epoch #{:>3d}'.format(epoch), leave=False):
        ...
        d_fake_loss = (adv_loss(adv_out, fake) + aux_loss(aux_out, fake_labels)) / 2
        ...
        g_loss = (adv_loss(adv_out, real) + aux_loss(aux_out, fake_labels)) / 2
        g_loss.backward()
        optimizer_g.step()
